...
LOG_MODULE_DECLARE(siwx91x_wifi, CONFIG_SIWX91X_WIFI_LOG_LEVEL);
...
void siwx91x_wifi_on_scan_results(const struct siwx91x_nwp_wifi_cb *ctxt, struct net_buf *buf)
{
	...
	int ies_length = buf->len - sizeof(struct siwx91x_frame_desc) - sizeof(struct ieee80211_beacon);

	__ASSERT(sizeof(payload->bssid) == WIFI_MAC_ADDR_LEN, "Corrupted build");
	__ASSERT(sizeof(tmp.mac) == WIFI_MAC_ADDR_LEN, "Corrupted build");
	if (!ieee80211_is_beacon(payload->frame_control) && !ieee80211_is_probe_resp(payload->frame_control)) {
		return;
	}
	...
	for (elt = (struct ieee80211_element *)payload->ies;
	     payload->ies + ies_length - (uint8_t *)elt >= sizeof(struct ieee80211_element) &&
	     payload->ies + ies_length - (uint8_t *)elt >= sizeof(struct ieee80211_element) + elt->datalen;
		      ...
		      struct wifi_scan_params *params, scan_result_cb_t cb)
{
	...
	chan_mask = 0;
	for (i = 0; i < ARRAY_SIZE(params->band_chan) && params->band_chan[i].channel; i++) {
		if (params->band_chan[i].band != WIFI_FREQ_BAND_2_4_GHZ) {
			continue;
		}
		chan_mask |= BIT(params->band_chan[i].channel - 1);
	}
	siwx91x_nwp_scan(config->nwp_dev, chan_mask, params->ssids[0],
