...
static inline struct ncp_spi_data *ncp_spi_data_from_drv(sli_cpc_drv_t *drv)
{
	return CONTAINER_OF(drv, struct ncp_spi_data, drv);
}
...
static void ncp_spi_header_done(const struct device *dev)
{
	...
	status = sli_cpc_alloc_rx_payload(data->drv.bus, rx);
...
static sl_status_t ncp_spi_drv_hw_init(sli_cpc_drv_t *drv)
{
...
static sl_status_t ncp_spi_drv_init(sli_cpc_drv_t *drv, sl_cpc_bus_t *bus)
{
	drv->bus = bus;
...
static sl_status_t ncp_spi_drv_start_rx(sli_cpc_drv_t *drv)
{
	const struct device *dev = drv->dev;
	const struct ncp_spi_config *cfg = dev->config;
	struct ncp_spi_data *data = dev->data;
...
static void ncp_spi_drv_deinit(sli_cpc_drv_t *drv)
{
	const struct device *dev = drv->dev;
	const struct ncp_spi_config *cfg = dev->config;
	struct ncp_spi_data *data = dev->data;
...
static sl_status_t ncp_spi_drv_read(sli_cpc_drv_t *drv, sli_cpc_frame_list_t *frames)
{
	struct ncp_spi_data *data = ncp_spi_data_from_drv(drv);
	unsigned int key;
...
static uint32_t ncp_spi_drv_get_available_write_frame_slots(sli_cpc_drv_t *drv)
{
	struct ncp_spi_data *data = ncp_spi_data_from_drv(drv);
	uint32_t used;
	unsigned int key;
...
static uint32_t ncp_spi_drv_write(sli_cpc_drv_t *drv, sli_cpc_frame_list_t *frames)
{
	struct ncp_spi_data *data = ncp_spi_data_from_drv(drv);
	...
	if (ncp_spi_drv_get_available_write_frame_slots(drv) < count) {
...
static void ncp_spi_drv_on_rx_frame_free(sli_cpc_drv_t *drv)
{
	ncp_spi_refill_rx(drv->dev);
}

static void ncp_spi_drv_get_local_capabilities(sli_cpc_drv_t *drv, const void **caps_p,
...
static sl_status_t ncp_spi_drv_set_remote_capabilities(sli_cpc_drv_t *drv, const void *caps,
						       uint16_t caps_size)
{
	struct ncp_spi_data *data = ncp_spi_data_from_drv(drv);
	...
	if (remote_speed != 0 && remote_speed < data->spi_cfg->frequency) {
		LOG_INF("%s: clamping SPI frequency to %u Hz", drv->dev->name, remote_speed);
...
static int ncp_spi_init(const struct device *dev)
{
	...
	sys_put_le32(cfg->spi.config.frequency, data->drv.local_caps.max_speed_le);
...
#define NCP_SPI_DEFINE(inst)                                                                       \
	static struct ncp_spi_data ncp_spi_data_##inst = {                                          \
		.common.cpc_bus = {                                                                \
			...
			.drv = &ncp_spi_data_##inst.drv,                                           \
			.drv_ops = &ncp_spi_drv_ops,                                               \
		},                                                                                 \
	};                                                                                         \
